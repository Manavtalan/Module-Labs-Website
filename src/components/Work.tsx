...
function ProjectVideo({project}:{project:Project}){
  ...
  return <video ref={videoRef} src={project.video} autoPlay muted loop playsInline preload="metadata" aria-label={project.videoLabel}/>;
}

function ProjectCard({project,duplicate=false}:{project:Project;duplicate?:boolean}){
  const titleId=`${duplicate?'duplicate':'primary'}-${project.id}-title`;
  const tagsId=`${duplicate?'duplicate':'primary'}-${project.id}-tags`;
  return <article className="work-project-entry" aria-labelledby={titleId}>
    <a
    ...
    >
      ...
      <span className="work-card-overlay">
        <span className="work-project-copy">
          ...
          <span className="work-project-tags" id={tagsId}>{project.tags.map(tag=><span key={tag}>{tag}</span>)}</span>
        </span>
...
export default function Work(){
  return <section id="work" className="section work-section" aria-labelledby="work-title">
    <div className="section-head reveal">
      <div><Label>SELECTED WORK</Label><h2 id="work-title">A few things we’ve<br/><em>put into the world.</em></h2></div>
